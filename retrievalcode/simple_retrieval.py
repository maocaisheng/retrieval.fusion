...
def main(feature_type, db_type, pretrain):
    # read database
    # feature_dim = 1024
#     add_noise_data = False
    ...
    cluster_num = 256
    if 'CONV4' in feature_type:
        db_feature_conv4 = os.path.join(work_dir,'feat','CONV4_db_cluster_%d.mat'%cluster_num)
        query_feature_conv4 = os.path.join(work_dir,'feat','CONV4_query_cluster_%d.mat'%cluster_num)
        feats, db_name_list = read_mat_feature(db_feature_conv4)
        feats_test, query_name_list = read_mat_feature(query_feature_conv4)
    ...
    elif 'RMAC' in feature_type:
        db_feature = os.path.join(work_dir,'feat','RMAC_db_pca_1024.h5')
        query_feature=os.path.join(work_dir,'feat','RMAC_query_pca_1024.h5')
        feats, db_name_list = read_feature(db_feature)
        feats_test, query_name_list = read_feature(query_feature)
        db_name_list = list(map(lambda x: x.decode().replace(replace_, ''), db_name_list))
        query_name_list = list(map(lambda x: x.decode().replace(replace_, ''), query_name_list))
    else:
        db_feature_conv4 = os.path.join(work_dir,'feat','CONV4_db_cluster_%d.mat'%cluster_num)
        query_feature_conv4 = os.path.join(work_dir,'feat','CONV4_query_cluster_%d.mat'%cluster_num)
...
#     i = query_name_list.index('copydays_original/205000.jpg')
#     line = [query_name_list[i]]
#     imgs=[]
...
#         img = Image.open(os.path.join(replace_, db_name_list[I[i,j]]))
#         img = trans(img)
#         imgs.append(img)
...
if __name__ == '__main__':
    main('CONV4','Challenging',pretrain=False)
